...
class Handle_post{
	...
	function post_handle_search($data_keys, $data_data, $data_lengths, $sql){
		$action = new Project_action;
		$action->init();	
		...
		echo json_encode($action->search($ret_data, $sql));
	}
	...
	function post_handle_post(){
			...
			if( $type == 21 ){
				...
				$data_lengths=$_POST['data_lengths'];
				$sql=$_POST['sql'];
				$this->post_handle_search($data_keys, $data_data, $data_lengths, $sql);
			}
	}
